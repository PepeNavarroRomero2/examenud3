...
export async function DELETE(request){
    ...
    const {data: deleteData, error} = await supabase.from("videojuego").delete().eq("id", id)
...
export async function PUT(request){
    const body = await request.json()
    const id = body.id
    const {data: updateData, error} = await supabase.from("videojuego").update(body.update).eq("id",id)
...
export async function POST(request){
    ...
    const {data: postData, error} = await supabase.from("videojuego").insert(juego)
